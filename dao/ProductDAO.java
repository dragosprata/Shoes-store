...
public class ProductDAO {
	...
	private static Product getProductFromResultSet(ResultSet rs) throws SQLException{
		...
		if(rs.next()){
			...
			product.setMarime(rs.getInt("MARIME"));
			product.setCuloare(rs.getString("CULOARE"));
		}
		return product;
	}
	//*******************************
	...
	public static void updateProdPrice(String prodId,String prodPrice) throws SQLException, ClassNotFoundException {
	 ...
	 //*************************************
    //UPDATE a product's colour
    //*************************************
public static void updateProdCuloare(String prodId, String prodCuloare) throws SQLException, ClassNotFoundException {
	//Declare a UPDATE statement
	String updateStmt = "UPDATE product SET CULOARE='"+prodCuloare+"' WHERE ID_PRODUS = '"+prodId+"'";
			//Execute UPDATE operation
			try{
				ConnectionFactory.dbExecuteUpdate(updateStmt);
			}catch (SQLException e){
    ...
    //INSERT a product
   //*************************************
	public static void insertProd(String denumire,String producator,String pret,String marime,String culoare) throws SQLException, ClassNotFoundException{
		//Declare a insert statement
		String insertStmt = "INSERT INTO product (DENUMIRE,PRODUCATOR,PRET,MARIME,CULOARE) VALUES ('"+denumire+"','"+producator+"','"+pret+"','"+marime+"','"+culoare+"')";
